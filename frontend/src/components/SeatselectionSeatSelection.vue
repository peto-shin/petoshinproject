<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            SeatSelection # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            SeatSelection
        </v-card-title >        
        ...
        <v-card-text>
            <String label="SeatId" v-model="value.seatId" :editMode="editMode" :inputUI="''"/>
...
<script>
    ...
    export default {
        name: 'SeatselectionSeatSelection',
        components:{
        },
        props: {
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/seatSelections'), this.value)
                        } else {
